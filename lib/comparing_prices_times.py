...
def get_morning_ex_price_changes(df):
    times_compare = [['1', '6'], ['1', '10'], ['6', '10']]
    for t in times_compare:
        df = df.pipe(calculations.calculate_pct_change_time,  f'{t[0]}_ex',
            f'{t[1]}_ex', 'change_ex', str(t[0]+'_'+t[1])) 
    return df
...
def get_morning_ex_price_change_ranks(df):
    times_compare = ['1_6', '1_10', '6_10']
    for t in times_compare:
        df = df.pipe(calculations.rank_by_race_ascending_time,  'change_ex_rank', 'time', 
            f'change_ex_{t}', t)
    return df
...
def get_morning_ex_starting_price_changes(df):
    times_compare = [['1', 'isp'], ['1', 'BSP'],['6', 'isp'], ['6', 'BSP'], ['10', 'isp'], ['10', 'BSP']]
    for t in times_compare:
        df = df.pipe(calculations.calculate_pct_change_time,  f'{t[0]}_ex', t[1],
            'change_ex', str(t[0]+'_'+t[1])) 
    return df
...
def check_if_fave_any(df):
    return (df
            .assign(any_fave=1)
            .assign(any_fave=lambda x : x.any_fave.where(
                x.loc[:, ['fave_1', 'fave_6', 'fave_10', 'fave_isp', 'fave_BSP']].max(axis=1) == 1 , 0)))
...
def all_fave(df):
    return (df
        .assign(all_fave=1)
        .assign(all_fave=lambda x : x.all_fave.where(
            x.loc[:, ['fave_1', 'fave_6', 'fave_10', 'fave_isp', 'fave_BSP']].sum(axis=1) == 5 , 0)))
