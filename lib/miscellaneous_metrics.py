...
def get_nearly_won(df):
    ...
    return (df
            .assign(nearly_won=1)
            .assign(nearly_won= lambda df: df.nearly_won.where((df.position == 2) & \
                (df.distance_behind < 0.5), None)))
...
def get_total_distance_behind(df):
    return (df
            ...
            .assign(total_distance_behind= lambda df: df.groupby(['time'], sort=True) \
                ['distance_behind'].transform(lambda x: x.rolling(40, min_periods=0).sum())))


def get_racetype(df):
    return df.assign(racetype = lambda df: df.EVENT_NAME.str.split(" ", n=1).str[1],
                     race_location = lambda df: df.MENU_HINT.str.split(" ").str[2])
...
def get_min_mean_max_metrics(df):
    ...
    return pd.merge(df, metrics, on='time', how='left').drop(columns=['MENU_HINT', 'EVENT_NAME'])
